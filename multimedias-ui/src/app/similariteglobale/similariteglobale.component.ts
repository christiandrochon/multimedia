...
@Component({
  selector: 'app-similariteglobale',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './similariteglobale.component.html',
  styleUrl: './similariteglobale.component.css'
})
export class SimilariteglobaleComponent implements OnInit{
  ...
  constructor(private similariteGlobaleService: SimilariteglobaleService) {
  }
  ...
  loadRandomImage() {
    ...
    this.similariteGlobaleService.getRandomImage().subscribe({
      next: (path: string) => {
  ...
  searchSimilarImages() {
    this.loading = true;
    this.similariteGlobaleService.getSimilarImages(this.queryImage, 8).subscribe(
      images => {
