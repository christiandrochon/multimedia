...
@Component({
  selector: 'app-similariteforme',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './similariteforme.component.html',
  styleUrl: './similariteforme.component.css'
})
export class SimilariteformeComponent implements OnInit {
  ...
  constructor(private similariteFormeService: SimilariteformeService) {
  }
  ...
  loadRandomImage() {
    ...
    this.similariteFormeService.getRandomImage().subscribe({
      next: (path: string) => {
  ...
  searchSimilarImages() {
    this.loading = true;
    this.similariteFormeService.getSimilarImages(this.queryImage, 8).subscribe(
      images => {
